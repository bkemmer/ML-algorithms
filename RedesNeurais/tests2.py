...
def inicializa_parametros(n_x, n_h, n_y, seed=42):
    """ Função para inicializar os parâmetros da rede

    Args:
        n_x (int): Tamanho da camada de entrada
        n_h (int): Tamanho da camada oculta
        n_y (int): Tamanho da camada de saída

    Returns:
        dict: dicionário com os parâmetros:
            W1 -- matriz de pesos (n_h x n_x)
            b1 -- vetor de bias
            W2 -- matriz de pesos (n_y x n_h)
            b2 -- vetor de bias
    """
    np.random.seed(seed)
...
def two_layer_model(X, Y, layers_dims, learning_rate = 0.05, num_iterations = 3000, print_cost=False):
    ...
    parameters = inicializa_parametros(n_x, n_h, n_y)
    ### END CODE HERE ###
    ...
    for i in range(0, num_iterations):
        ...
        parameters = atualiza_parametros(parameters, grads, learning_rate)
        ### END CODE HERE ###
